...
const ProfileSection = () => {
  ...
  const handleListItemClick = (event, index, route = '') => {
    ...
    if (route && route !== '') {
      navigate(route);
    } else if (index === 0) { // Login button
      navigate('/pages/login/login3');
    } else if (index === 1) { // Register button
            ...
            <Paper>
              <ClickAwayListener onClickAway={handleClose}>
                {isLoggedIn ? (
                  ...
                  <MainCard border={false} elevation={16} content={false} boxShadow shadow={theme.shadows[16]}><Box sx={{ p: 2 }}>
                      <List
                        component="nav"
                        sx={{
                          width: '100%',
